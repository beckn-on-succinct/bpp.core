...
import in.succinct.bpp.core.db.model.Order;
...
public class OrdersController extends ModelController<Order> {
    ...
    protected Expression getWhereClause(){
        ...
        if  (user.getId() > 1) {
            ...
            if (!ObjectUtil.isVoid(providerId)) {
                addl.add(new Expression(getReflector().getPool(), "PROVIDER_ID", Operator.EQ, providerId));
            }else {
                addl.add(new Expression(getReflector().getPool(), "PROVIDER_ID", Operator.EQ));
            }
            where.add(addl);
        }
    ...
    public View download_orders(){
        ...
        Select q = new Select().from(Order.class);
        ...
        List<Order> orders =  q.where(where).execute(Order.class,0,getFilter());
        
        Workbook wb = new XSSFWorkbook();
        super.exportxls(Order.class, wb,orders);
